...
class Attention(nn.Module):
    ...
    def pad_and_reshape_tok_attn(self, tok_attn, target_length=256):
        ...
        Args:
            tok_attn (torch.Tensor): Attention maps with shape [batch_size, num_heads, num_keypoints, current_length].
            target_length (int): Target length to pad to.

        Returns:
            torch.Tensor: Padded and reshaped attention maps with shape [batch_size, num_heads, num_keypoints, 16, 16].
        """
        batch_size, num_keypoints, current_length = tok_attn.shape
        padding_length = target_length - current_length
        if padding_length > 0:
            # Pad the last dimension to target_length
            tok_attn = F.pad(tok_attn, (0, padding_length), value=0)

        # Reshape the padded attention map to 16x16
    ...
    def visualize_tok_attn(self,tok_attn, keypoint_index=0, sample_index=0):
        ...
        Args:
            tok_attn (torch.Tensor): Attention maps with shape [batch_size, num_keypoints, num_patches].
            keypoint_index (int): Index of the keypoint to visualize.
            sample_index (int): Index of the sample in the batch to visualize.
        """
        # Select the attention map for the specified keypoint and sample
        keypoint_attn = tok_attn[sample_index, keypoint_index, :].detach().cpu().numpy()

        # Reshape the attention map to a 2D array (16x16)
        keypoint_attn_2d = keypoint_attn.reshape((16, 16))

        # Visualize the attention map
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.heatmap(keypoint_attn_2d, ax=ax, cmap='viridis', annot=False)  # Set annot=False to remove numbers
        ax.set_title(f'Attention Map for Keypoint {keypoint_index} in Sample {sample_index}')
        plt.show()
...
class SDPose(nn.Module):
    ...
    def forward(self, feature, mask=None, ratio=0.7):
        ...
        for i in range(self.cycle_num):
            if i == 1:
                flag=False
            x, attn_x, pos = self.transformer_layer1(x, mask, self.pos_embedding, prune=flag, keep_ratio=ratio, pass_1_pos=pos)
            y1 = EasyDict(
                vis_token=x[:, 0:self.num_keypoints],
                kpt_token=x[:, self.num_keypoints:]
            )
            kpt_token = x[:, 0:self.num_keypoints]
            vis_token = x[:, self.num_keypoints:]
            temp_res = self.to_keypoint_token(kpt_token)
            temp_res = self.mlp_head(temp_res)
            temp_res = rearrange(temp_res, 'b c (p1 p2) -> b c p1 p2', p1=self.heatmap_size[0], p2=self.heatmap_size[1])
            output = EasyDict(
                y1=y1,
                vis_token=vis_token,
                kpt_token=kpt_token,
...
class RLESDPose_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, 
                 mlp_ratio, apply_init=False, apply_multi=True, hidden_dim=384,
                 channels = 3, dropout = 0., emb_dropout = 0., pos_embedding_type="learnable", cycle_num = 2):
        super().__init__()
        """
        DistilPose base head.
        Basically the same as TokenPose_base, regression-based prediction head
        ...
        # transformer
        self.transformer = Transformer_sd(dim, depth, heads, mlp_dim, dropout,
                                       num_keypoints=num_keypoints, all_attn=self.all_attn,
                                       scale_with_head=True)
