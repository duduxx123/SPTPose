...
from .utils.tokenbase import RLESDPose_base

BN_MOMENTUM = 0.1

@HEADS.register_module()
class RLESDPoseHead(nn.Module):
    """RLESDPose head.
    ...
    def __init__(self,
        ...
        self.tokenhead = RLESDPose_base(feature_size=tde_cfg.feature_size, 
    ...
    def get_loss(self, output, target, target_weight):
        ...
        Args:
            output (dict): student output 
                ...
                - output.pred (torch.Tensor[N, K, 4]): Output regression
        ...
        # print(target.dim(),target.shape)  # 3 torch.Size([64, 17, 2])
        assert target.dim() == 3 and target_weight.dim() == 3

        output_len = len(output)

        for i in range(output_len):
            losses['reg_loss'] = self.kpt_loss(output[i].pred, target, target_weight)
    ...
    def decode(self, img_metas, output, **kwargs):
        ...
        preds, _ = keypoints_from_regression(output, c, s,
                                                   kwargs['img_size'])

        all_preds = np.zeros((batch_size, preds.shape[1], 3), dtype=np.float32)
        all_boxes = np.zeros((batch_size, 6), dtype=np.float32)
        all_preds[:, :, 0:2] = preds[:, :, 0:2]
        all_preds[:, :, 2:3] = (1 - sigma).mean(axis=-1, keepdims=True)
        all_boxes[:, 0:2] = c[:, 0:2]
        all_boxes[:, 2:4] = s[:, 0:2]
